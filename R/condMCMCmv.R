...
condMCMCmv <- function(data, grid = NULL, niter, nburn,  m0 = NULL, k0 = NULL, 
                       ...
                       process = "DP", sigma_PY = 0, print_message = FALSE){
  ...
  if(method == "CPU"){
    if(process == "DP"){
      est_model <- cPUS_mv(data, grid_use, niter, nburn, m0, k0, S0, n0, 
                           mass, napprox, nupd, out_param, out_dens, 0, sigma_PY, print_message)  
    } else if(process == "PY"){
      est_model <- cPUS_mv(data, grid_use, niter, nburn, m0, k0, S0, n0, 
                           mass, napprox, nupd, out_param, out_dens, 1, sigma_PY, print_message)  
    } 
  } else if(method == "SLI"){
    if(process == "DP"){
...
setMethod(f = "plot",
          ...
          definition = function(x, prob = c(0.025, 0.975), dimension = c(1,1), col = "#0037c4"){
            ...
            if(dimension[1] == dimension[2]){
              ...
              ggplot2::ggplot(plot_df_use, aes(x = Group.1, y = V1)) + 
                ...
                geom_line(aes(x = Group.1, y = V2), size= 1, color = col)
