...
void simu_trunc_PUS(arma::vec &mutemp,
  ...
  // s2temp[0] = 1.0 / arma::randg(arma::distr_param(a0, 1.0 / b0));
  // mutemp[0] = arma::randn() * sqrt(k0 * s2temp[0]) + m0;
  freqtemp.fill(1);
  int k = 1;
  ...
  for(int j = 1; j < napprox; j++){
    ...
    if(temp_cl < (k - 1)){
      ...
      // s2temp[k] = 1.0 / arma::randg(arma::distr_param(a0, 1.0 / b0));
      // mutemp[k] = arma::randn() * sqrt(k0 * s2temp[temp_cl]) + m0;
      freqtemp[k] = 1;
      k += 1;
    }
  }
...
void simu_trunc_PUS_mv(arma::mat &mutemp,
                       ...
                       int napprox){
  // resize the objects to size 1
  // mutemp.resize(1, mutemp.n_cols);
  // s2temp.resize(s2temp.n_rows, s2temp.n_cols, 1);
  freqtemp.resize(1);

  // initialize the first element
  // s2temp.slice(0) = arma::inv(arma::wishrnd(arma::inv(S0), n0));
  // mutemp.row(0) = arma::trans(arma::mvnrnd(m0, s2temp.slice(0)/k0));
  freqtemp.fill(1);
  int k = 1;
  ...
  for(int j = 1; j < napprox; j++){
    ...
    if(temp_cl < (k - 1)){
      ...
      // if is a new one, generate the new parameters
      // mutemp.resize(k + 1, mutemp.n_cols);
      // s2temp.resize(s2temp.n_rows, s2temp.n_cols, k + 1);
      freqtemp.resize(k + 1);
      
      // s2temp.slice(k) = arma::inv(arma::wishrnd(arma::inv(S0), n0));
      // mutemp.row(k) = arma::trans(arma::mvnrnd(m0, s2temp.slice(k)/k0));
      freqtemp[k] = 1;
      k += 1;
    }
  }
